...
router.on('login', () => {
    loading()
	view('login', {}, 'main', () => {
		$('#login--alert-is-error').hide()
		$('#button--login-submit').click(() => {
			let username = $('#inputUsername').val()
			let password = $('#inputPassword').val()
			if(username.length < 1 || password.length < 1) { return }
			$.post('/api/v1/login', {username: username, password: password}, (data) => {
				window.location.href = '/'
			}).fail(() => {
				$('#login--alert-is-error').show()
			})
		})
	})
})
    ...
    view('create-user', {}, 'main', () => {
        $('#button--create-user').click(() => {
            ...
            $.post('/api/v1/users/create', {
                username: username,
                password: password,
                write_permission: write_permission_bool
            }, (response) => {
                if (response.success) {
